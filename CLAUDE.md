...
|---------|---------|--------|
| VAPI | Voice AI platform for live coaching conversations | **LIVE** — Assistant `401905cf` published with full ~63K system prompt (John persona + Pre-Session Brief + Personal Goals + Profile). ElevenLabs Josh voice, Claude Sonnet 4. Phone: +1 (571) 498-9194. First message: "Hey Evan, it's John. How you doing?" Config at `agents/vapi-config.md`. **API PATCH push works for large prompts via `agents/update_vapi_prompt.py`** — uses `http.client` + custom User-Agent to bypass the Cloudflare WAF that blocks urllib. API key auto-loads from `.vapi_key` (project root, gitignored, chmod 600). Dashboard UI is the fallback, not the default. |
...
- [x] **VAPI agent setup** — DONE (2026-04-08). Assistant LIVE: ID `401905cf`, ElevenLabs Josh voice, Claude Sonnet 4. Phone: +1 (571) 498-9194. Persona: "John" — straight-shooter, not cheerleader. First message: "Hey Evan, it's John. How you doing?" **Update 2026-05-11:** API PATCH push at 63K chars confirmed working via `agents/update_vapi_prompt.py` (http.client + custom User-Agent bypasses Cloudflare WAF). API key auto-loads from project-root `.vapi_key` file. Dashboard UI is the fallback, not the default.
...
| 2026-04-20 | **PAYMENT TIMING RULE CHANGED.** Jude and Evan agreed (post-Session 2 coaching call) that payment is now requested AFTER the second meeting (Technical Discovery with Logan), NOT before it. New flow: Pitch → Tech Discovery → Payment link + LOE → Engagement. Old rule (payment as gate between Assessment and Tech Discovery) is retired. Post-pitch follow-up emails offering Tech Discovery booking links are now CORRECT behavior. The Session 2 weekly brief (`outputs/pre-session-brief-2026-04-20.txt`) coached against the old rule — post-session reports should not re-assert that coaching point. |
| 2026-05-11 | **Stage-specific-evaluation.md cleanup — old payment-first rule fully retired.** Session 3 weekly prep surfaced that stale rules were still in `references/stage-specific-evaluation.md` after the April 20 process change. Removed: Stage 2 "PAYMENT FIRST" next-steps line, post-pitch "Payment link included" follow-up requirement, standalone "CRITICAL PROCESS RULE" quote, and the entire Stage 2→3 GATE: PAYMENT COLLECTION section. Replaced with: a Stage 2 → Stage 3 "Booking the Tech Discovery" section under the current process, plus a PROCESS HISTORY block documenting why/how the rule changed (Evan's Session 2 pushback → proposal → company adoption). The history is intentionally kept as a teaching artifact — Decision Ladder 4-5 / Motivation Ladder Level 4 case study John can reference when similar friction surfaces. **Two-part sale officially documented:** (A) Sales Pitch — Evan solo unless the lead was a personal referral to Jude (then Jude leads by design); (B) Technical Discovery — Logan leads technical, Evan's #1 job is trust transfer to Logan via a full ~45-second intro (original founder / 20 yrs SR&ED / CPA from KPMG / dental software origin / personally recruited by Alex Popa as one of first 5 at Boast / "you are in good hands"). Trust transfer + post-Tech-Disco Stripe payment link added as the primary evaluation criteria at Stage 3. |
| 2026-05-11 | **SSI close → comp plan change for Evan (already announced to Evan on Fri May 8 by Jude).** Evan brought SSI in. Both Jude and Logan said NO at first. Evan stayed patient, kept pushing the lead, pushed leadership when he needed to, and closed it on the LAST day of FY2026 — that close landed the company's sales targets for the year. As a result, Jude changed Evan's comp plan and told Evan on Friday May 8. **Coaching note for John:** Evan knows about the comp change. A specific, earned congrats from John during the Session 3 call is welcome — IN CHARACTER (straight-shooter, not corporate cheer). Frame: the comp bump is the consequence, the behavior is what Evan controls and what gets coached. SSI is paired with the payment-process-change story as a second Motivation Level 4 case study — value alignment externally (SSI) and internally (payment process), proving the pattern. |
| 2026-05-11 | **VAPI auth + API push at 62.9K confirmed working — old "dashboard only" note retired.** VAPI private API key (`04a0437d-1d21-473f-...`) stored at project-root `.vapi_key` (chmod 600, gitignored). `update_vapi_prompt.py` updated to auto-load from `.vapi_key` if `VAPI_API_KEY` env var is not set — future weekly preps now run end-to-end without manual key handling. Session 3 push pushed 62,896 chars via API PATCH successfully — the http.client + custom User-Agent ("SRED-SalesCoach/1.0") approach bypasses the Cloudflare WAF that previously blocked urllib at smaller sizes. **Update the operational note in the Key Integrations table: API PATCH now works for large prompts via this script; dashboard UI is the fallback, not the default.** |
